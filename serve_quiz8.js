...
const { body, param, validRes } = require('express-validator');
...
const validateCompany = [body('companyIDCode').exists().withMessage('Company ID is required and cannot be empty').notEmpty().trim().escape(),body('companyName').exists().withMessage('companyName is missing').trim().escape(),body('companyCity').exists().withMessage('companyCity is missing').trim().escape()];
 ...
 * /company:
 *     post:
 *         summary: Insert a new company details
 *         description: An object of the company is added into the array od company objects
 *         requestBody:
 *                required: true
 *                content:
 *                   application/json:
 ...
 *         responses:
 *               200:
 *                  description: Added into Company
 *                  content:
 *                  application/json:
 *                      schema:
 *                          type: object
 ...
 *                              $ref: '#components/schemas/CompanyPost'
 *               500:
 *                  description: Error while inserting the company
 */
 ...
 * /company/{id}:
 *    patch:
 *       description: API is used to update only a specific fields to specified company row
 *       parameters:
 *          - in: path
 *            name: id
 *            required: true
 ...
 *       responses:
 *          200:
 *            description: Company updated with Name
 *            content:
 *                  application/json:
 *                      schema:
 *                          type: object
 ...
 */
  ...
  app.patch('/company/:id', validateCompanyID, validateCompanyPatch ,async (req, res) => {
    ...
    if (!errors.isEmpty()) {
     ...
     try {
         const reqbody = req.body; 
         const result = await pool.query("UPDATE company SET  COMPANY_NAME = '"+reqbody.companyName+"' WHERE COMPANY_ID = '"+ req.params.code +"'");
 ...
 * /company/{id}:
 *    delete:
 *       description: API is used to delete record from company table
 *       parameters:
 *          - in: path
 *            name: id
 *            required: true
 ...
 */
  app.delete('/company/:id', validateCompanyID,async (req, res) => {
    ...
    if (!errors.isEmpty()) {
     ...
     try {    
         const result = await pool.query("DELETE FROM company WHERE COMPANY_ID = '"+req.params.code+"'");
